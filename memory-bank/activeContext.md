...
1.  **Tauri コマンドとストレージの完全な統合**: (進行中)
    *   ~~`settings` コマンドハンドラを修正し、`AppSettings` リポジトリを使用するようにする。~~ (完了)
    *   各コマンドハンドラでのエラーハンドリング (`map_err` など) を見直し、`StorageError` から各コマンド固有エラーへの変換を確実に行う。(要確認)
2.  **`AppSettings` リポジトリの実装**: (完了)
    *   ~~`src-tauri/src/models/settings.rs` を作成。~~ (完了)
    *   ~~`storage/repository/settings_repository.rs` を作成し、設定の保存・取得ロジックを実装。~~ (完了)
