...
2.  **Tauri コマンドとストレージの統合**: 新しく実装されたストレージリポジトリを Tauri コマンドハンドラから呼び出し、エラーハンドリングを適切に行う必要があります。(`settings` コマンドは未対応)
3.  **ドキュメント同期とイベント処理**: `iroh-docs` の `subscribe` 機能を利用してドキュメントの変更を監視し、フロントエンドにリアルタイムで反映させる仕組みを実装する必要があります。
4.  **UIの改善**: 基本的なUIは実装されていますが、ユーザーエクスペリエンスの向上のために改善が必要です。(変更なし)
5.  **テストの実装**: ユニットテスト、統合テスト、E2Eテストを実装する必要があります。(変更なし)
6.  **パフォーマンスの最適化**: データ同期やP2P通信のパフォーマンスを最適化する必要があります。(変更なし)
7.  **AppSettings の実装**: アプリケーション設定を保存するためのストレージ実装が必要です。
...
2.  **Tauri コマンドの更新**:
    ...
    *   `settings` コマンド内のストレージ呼び出しは一時的にコメントアウト。
...
1.  **Tauri コマンドとストレージの完全な統合**:
    *   `settings` コマンドハンドラを修正し、`AppSettings` リポジトリ (未実装) を使用するようにする。
    *   各コマンドハンドラでのエラーハンドリング (`map_err` など) を見直し、`StorageError` から各コマンド固有エラーへの変換を確実に行う。
2.  **`AppSettings` リポジトリの実装**:
    *   `src/models/settings.rs` (または相当するファイル) を確認・作成。
    *   `storage/repository/settings_repository.rs` を作成し、設定の保存・取得ロジックを実装。
3.  **ドキュメント同期とイベント処理の実装**:
...
5.  **ストレージ技術**: (更新)
    ...
    *   固定の `NamespaceId` を使用して User と Post のドキュメントを分離 (現在の実装)。
...
5.  **テスト戦略**: (変更なし)
    ...
    *   P2P通信のテスト方法
6.  **Namespace 管理**: (新規)
    *   現在使用している固定の Namespace ID は適切か？将来的にユーザーごと、または他の基準で Namespace を分割する必要はあるか？
...
10. **Namespace 管理戦略**: (新規) User と Post で固定 Namespace を使用する現在の方法は適切か？将来的な拡張性を考慮すると、どのような管理戦略が望ましいか？