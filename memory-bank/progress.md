...
プロジェクトは**基本実装段階**にあります。基本的なアプリケーション構造と主要機能のバックエンドロジック（コマンド）は存在します。**iroh を使用したストレージレイヤーの再実装が完了**し、モック実装から本格的な実装への移行が完了しました。
iroh-gossipの実装は進行中ですが、検証が必要です。
次のステップは、Tauri コマンドとストレージの完全な統合、AppSettings リポジトリの実装、ドキュメント同期/イベント処理の実装、iroh-gossipの実装完成、UIの改善、テストの実装です。
...
- 🔄 **Tauri コマンドとストレージの統合**:
    - 🔄 `settings` コマンドハンドラの修正 (`AppSettings` リポジトリ待ち)
    - 🔄 各コマンドハンドラのエラーハンドリング (`map_err`) の見直し
- 🔄 **iroh-gossipの実装完成**:
    - ✅ 基本的なP2P通信の実装
    - ✅ トピックベースのメッセージングの実装
    - 🔄 実際のP2P通信のテストと検証
...
1.  **`AppSettings` リポジトリの実装**:
    *   `src/models/settings.rs` (または相当) を確認・作成。
    *   `storage/repository/settings_repository.rs` を作成し、設定の保存・取得ロジックを実装。
    *   `settings` コマンドハンドラを更新。
2.  **ドキュメント同期とイベント処理の実装**:
...
2.  **テストの不足**: ユニットテスト、統合テスト、E2Eテストが十分に実装されていないため、品質保証が不十分です。特にP2P通信のテスト方法の確立が必要です。(変更なし)
...
9.  **`settings` コマンド未対応**: `AppSettings` リポジトリが未実装のため、関連する Tauri コマンドが機能しない。(新規)
...
- ✅ **iroh-docsを使用した実際のデータ同期 (基本 CRUD 実装完了)**
